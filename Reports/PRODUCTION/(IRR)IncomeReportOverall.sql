/*
Income Report - New Overall Report

--Updated to take 3.6725, for AED currencies.
--Updated Total Brokerage - As per the calculation.
--Updated The report to have the Invoices which are not approved too.
...
SELECT 
    ...
    InquiryFuelDetailId,
	CASE 
		WHEN VatAmount is not null and VatAmount != 0 THEN VatAmount * aim.ExchangeRate
		ELSE
        (SELECT TOP 1 ISNULL(VatAmount, 0)
         ...
         WHERE sub.InquiryDetailId = aic.InquiryDetailId 
		   ...
		   AND sub.HasVat = 1
         ORDER BY sub.Id) * aim.ExchangeRate
	end AS VatAmount
FROM AppInvoiceCustomers aic
LEFT JOIN AppInquiryMargins aim On aim.Id = aic.InquiryMarginId and aim.IsDeleted = 0
WHERE aic.IsDeleted = 0 AND InvoiceType = 0 and MergeCode is not null AND HasVat = 1--and InquiryDetailId = '4A7DCC8D-F4AB-0876-32F4-3A185D35E261'
),
...
AdditionalCostCustomerCTE AS (
SELECT 
    SUM(AmountUsd * QtyCte) AS AmtxBDN,
    SUM(QtyCte) AS TotalBDN,
	(SUM(AmountUsd * QtyCte) / 
     SUM(SUM(QtyCte)) OVER (PARTITION BY aic.InquiryDetailId)) AS AdditionalCost,
    aic.InquiryDetailId,
    aic.InquiryFuelDetailId
FROM AppInvoiceCustomers aic
LEFT JOIN AppInvoiceAdditionalCosts aiac ON aic.InquiryDetailId = aiac.InquiryDetailId AND aiac.IsDeleted = 0
LEFT JOIN QtyCTE qc ON qc.Id = aic.InquiryFuelDetailId
...
SELECT 
    ...
    SUM(QtyCte) AS TotalBDN,
	(SUM(AmountUsd * QtyCte) / 
     SUM(SUM(QtyCte)) OVER (PARTITION BY ais.InquiryDetailId)) AS AdditionalCost,
    ais.InquiryDetailId,
    ais.InquiryFuelDetailId
FROM AppInvoiceSellers ais
LEFT JOIN AppInvoiceAdditionalCosts aiac ON ais.InquiryDetailId = aiac.InquiryDetailId AND aiac.IsDeleted = 0
LEFT JOIN QtyCTE qc ON qc.Id = ais.InquiryFuelDetailId
...
SELECT 
    ...
    aic.InquiryDetailId,
    InquiryFuelDetailId,
    ISNULL(Discount * aim.ExchangeRate, 
        (SELECT TOP 1 ISNULL(Discount, 0)
         ...
         ORDER BY sub.Id) * aim.ExchangeRate ) AS Discount
FROM AppInvoiceCustomers aic
LEFT JOIN AppInquiryMargins aim On aim.Id = aic.InquiryMarginId and aim.IsDeleted = 0
WHERE aic.IsDeleted = 0 AND InvoiceType = 0
),
...
SELECT DISTINCT
	...
	CASE 
		WHEN aid.InquiryStatus IN (700,800) THEN ROUND((ain.QuantityMax * aim.SellPriceUsd) + ISNULL(mcc.TotalMiscCost, 0), 2)
		ELSE ROUND(ISNULL(((aic.SellPrice * aim.ExchangeRate) * aic.BdnBillingQuantity),0) + ISNULL(aic.BuyerGradeMiscCost, 0) + ISNULL(acc.AdditionalCost, 0) - ISNULL(pdc.PrepaymentDiscount, 0), 2)
	END AS 'Customer Invoice Amount',
	CASE 
		WHEN aid.InquiryStatus IN (700,800) THEN ROUND((ain.QuantityMax * aim.SellPriceUsd) + ISNULL(mcc.TotalMiscCost, 0), 2)
		ELSE ROUND(ISNULL(((aic.SellPrice * aim.ExchangeRate) * aic.BdnBillingQuantity),0) + ISNULL(aic.BuyerGradeMiscCost, 0) + ISNULL(acc.AdditionalCost, 0) - ISNULL(pdc.PrepaymentDiscount, 0) + ISNULL(vc.VatAmount, 0), 2)
	END AS 'Customer Invoice Total Amount',
	ROUND(aic.AmountReceivedSoFar * aim.ExchangeRate, 2) AS 'Amount Received',
	ROUND(COALESCE(aic.SellPrice * aim.ExchangeRate, aim.SellPriceUsd),2) AS 'Selling Price',
...
SELECT 
	...
	BdnBillingQuantity,
	tcte.MergeCode,
	AmountReceivedSoFar * aim.ExchangeRate AS AmountReceivedSoFar
	--CASE 
...
LEFT JOIN AppInquiryMargins aim On aim.Id = aic.InquiryMarginId and aim.IsDeleted = 0
WHERE aic.IsDeleted = 0 AND InvoiceType = 0 and aic.MergeCode is not null--and InquiryDetailId = '4A7DCC8D-F4AB-0876-32F4-3A185D35E261'
)
,
TotalAmountCTE AS (
Select 
	aic.InquiryDetailId,
	SUM(TotalAmount * aim.ExchangeRate) as 'TotalAmountAdjusted',
	SUM(SubTotal * aim.ExchangeRate) as 'SubTotalAdjusted'
from AppInvoiceCustomers aic
LEFT JOIN AppInquiryMargins aim On aim.Id = aic.InquiryMarginId and aim.IsDeleted = 0
...
SELECT 
	...
	BdnBillingQuantity,
	CASE 
		WHEN (AmountReceivedSoFar) is not null and AmountReceivedSoFar != 0 THEN (AmountReceivedSoFar * aim.ExchangeRate)
		ELSE
        (SELECT TOP 1 ISNULL(AmountReceivedSoFar * aim.ExchangeRate, 0)
         FROM AppInvoiceCustomers AS sub
         WHERE sub.InquiryDetailId = aic.InquiryDetailId 
           AND sub.IsDeleted = 0 
           AND sub.InvoiceType = 0 
...
FROM AppInvoiceCustomers aic
LEFT JOIN AppInquiryMargins aim On aim.Id = aic.InquiryMarginId and aim.IsDeleted = 0
WHERE aic.IsDeleted = 0 AND InvoiceType = 0 and MergeCode is null--and InquiryDetailId = '4A7DCC8D-F4AB-0876-32F4-3A185D35E261'
...
SELECT DISTINCT
	...
	--ROUND(aic.SubTotal * aim.ExchangeRate, 2) AS 'Customer Invoice Amount',
	CASE 
		WHEN aid.InquiryStatus IN (700,800) THEN ROUND((ain.QuantityMax * aim.SellPrice) + ISNULL(mcc.TotalMiscCost, 0), 2)
		ELSE ROUND(ISNULL(((aic.SellPrice) * aic.BdnBillingQuantity),0) + ISNULL(aic.BuyerGradeMiscCost, 0) + ISNULL(acc.AdditionalCost, 0) - ISNULL(pdc.PrepaymentDiscount, 0), 2)
	END AS 'Customer Invoice Amount',
	CASE 
		WHEN aid.InquiryStatus IN (700,800) THEN ROUND((ain.QuantityMax * aim.SellPrice) + ISNULL(mcc.TotalMiscCost, 0), 2)
		ELSE ROUND(ISNULL(((aic.SellPrice) * aic.BdnBillingQuantity),0) + ISNULL(aic.BuyerGradeMiscCost, 0) + ISNULL(acc.AdditionalCost, 0) - ISNULL(pdc.PrepaymentDiscount, 0) + ISNULL(COALESCE(vc.VatAmount, vc.VatAmount), 0), 2)
	END AS 'Customer Invoice Total Amount',
...
SELECT DISTINCT
	...
	CASE 
		WHEN aifd.IsCancelled = 1 THEN NULL
		ELSE ROUND(aim.Margin, 2)
	...
	CASE 
		...
		--ELSE ROUND(ISNULL(((aic.SellPrice * aim.ExchangeRate) * aic.BdnBillingQuantity),0) + ISNULL(aic.BuyerGradeMiscCost, 0) + ISNULL(acc.AdditionalCost, 0) - ISNULL(pdc.PrepaymentDiscount * aim.ExchangeRate, 0) - ISNULL(aic.Discount * aim.ExchangeRate, 0), 2)
		WHEN aid.InquiryStatus NOT IN (700,800) AND sc.Code = 'AED' THEN ROUND(aic.SubTotal / 3.6725, 2) 
		WHEN aid.InquiryStatus NOT IN (700,800) AND sc.Code <> 'AED' THEN ROUND(aic.SubTotal * aim.ExchangeRate, 2)
	END AS 'Customer Invoice Amount',
	CASE 
		WHEN aid.InquiryStatus IN (700,800) AND sc.Code = 'AED' THEN ROUND((ain.QuantityMax * (aim.SellPrice / 3.6725)) + ISNULL(mcc.TotalMiscCost, 0), 2)
		WHEN aid.InquiryStatus IN (700,800) AND sc.Code <> 'AED' THEN ROUND((ain.QuantityMax * aim.SellPriceUsd) + ISNULL(mcc.TotalMiscCost, 0), 2)
		--ELSE ROUND(ISNULL(((aic.SellPrice * aim.ExchangeRate) * aic.BdnBillingQuantity),0) + ISNULL(aic.BuyerGradeMiscCost, 0) + ISNULL(acc.AdditionalCost, 0) - ISNULL(pdc.PrepaymentDiscount * aim.ExchangeRate, 0) - ISNULL(aic.Discount * aim.ExchangeRate, 0), 2)
		WHEN aid.InquiryStatus NOT IN (700,800) AND sc.Code = 'AED' THEN ROUND(aic.TotalAmount / 3.6725, 2) 
		WHEN aid.InquiryStatus NOT IN (700,800) AND sc.Code <> 'AED' THEN ROUND(aic.TotalAmount * aim.ExchangeRate, 2) 
	END AS 'Customer Invoice Total Amount',
	ais.InvoiceNumber AS 'Seller Invoice Number',
	--CASE 
	--	WHEN aid.InquiryStatus IN (700,800) THEN (ain.QuantityMax * aim.BuyPriceUsd)
	--	ELSE ROUND(ISNULL(((ais.BuyPrice * aio.ExchangeRate) * ais.BdnBillingQuantity),0) + ISNULL(ais.BuyerGradeMiscCost, 0) + ISNULL(acs.AdditionalCost, 0) - ISNULL(ais.Discount * aio.ExchangeRate, 0), 2) 
	...
	CASE 
		...
		WHEN aid.InquiryStatus NOT IN (700,800) AND bc.Code <> 'AED' THEN ROUND(ais.SubTotal * aio.ExchangeRate, 2) 
	END AS 'Seller Invoice Amount',
	CASE 
		WHEN aid.InquiryStatus IN (700,800) AND bc.Code = 'AED' THEN ROUND(((aim.BuyPrice/3.6725) * ain.QuantityMax)  + ISNULL(mcs.TotalMiscCost, 0),2)
		WHEN aid.InquiryStatus IN (700,800) AND bc.Code <> 'AED' THEN ROUND((aim.BuyPriceUsd * ain.QuantityMax)  + ISNULL(mcs.TotalMiscCost, 0),2)
		WHEN aid.InquiryStatus IN (700,800) AND bc.Code = 'AED' THEN ROUND(ais.TotalAmount / 3.6725, 2)
		WHEN aid.InquiryStatus IN (700,800) AND bc.Code <> 'AED' THEN ROUND(ais.TotalAmount * aio.ExchangeRate, 2) 
	END AS 'Seller Invoice Total Amount',
	--ROUND(COALESCE(ars.AmountReceivedReal, arss.AmountReceivedReal, aic.AmountReceivedSoFar * aim.ExchangeRate),2) AS 'Amount Received',
	CASE 
		WHEN sc.Code = 'AED' THEN ROUND(aic.AmountReceivedSoFar / 3.6725,2)
		WHEN sc.Code <> 'AED' THEN ROUND(aic.AmountReceivedSoFar * aim.ExchangeRate,2) 
	...
	COALESCE(ais.BuyPrice * aio.ExchangeRate, aim.BuyPriceUsd) AS 'Buying Price',
	bc.Code AS 'Buying Currency',
	COALESCE(aic.SellPrice * aim.ExchangeRate, aim.SellPriceUsd) AS 'Selling Price',
	sc.Code AS 'Selling Currency',
	ISNULL(
    CASE 
...
WHERE aid.InquiryStatus IN (650,700,800,900,1000,9000) OR ( ais.InvoiceType is null) --aic.InvoiceStatus NOT IN (0,10,20) OR ais.InvoiceStatus NOT IN (0,10,20) OR) --and aid.Code = 'G2958'
)
,
CustomerInvoices AS (
    SELECT 
		...
		END AS 'Customer Invoice Amount',
		CASE 
			WHEN aic.InvoiceType = 1 AND acur.Code = 'AED' THEN ROUND(-ABS(aic.TotalAmount / 3.6725),2)
			WHEN aic.InvoiceType = 1 AND acur.Code <> 'AED' THEN ROUND(-ABS(aic.TotalAmount * aim.ExchangeRate),2)
			WHEN aic.InvoiceType = 2 AND acur.Code = 'AED' THEN ROUND(aic.TotalAmount / 3.6725, 2)
			WHEN aic.InvoiceType = 2 AND acur.Code <> 'AED' THEN ROUND(aic.TotalAmount * aim.ExchangeRate, 2)
		END AS 'Customer Invoice Total Amount',
		CASE 
			WHEN aic.InvoiceType = 1 AND acur.Code = 'AED' THEN ROUND(-ABS(aic.AmountReceivedSoFar / 3.6725),2)
			WHEN aic.InvoiceType = 1 AND acur.Code <> 'AED' THEN ROUND(-ABS(aic.AmountReceivedSoFar * aim.ExchangeRate),2)
			WHEN aic.InvoiceType = 2 AND acur.Code = 'AED' THEN ROUND(aic.AmountReceivedSoFar / 3.6725, 2)
			WHEN aic.InvoiceType = 2 AND acur.Code <> 'AED' THEN ROUND(aic.AmountReceivedSoFar * aim.ExchangeRate, 2)
		END AS 'Amount Received',
		CASE
			WHEN aid.InquiryStatus IN (700,800) THEN 'Not received'
			WHEN aid.InquiryStatus NOT IN (700,800)
...
--Select * from CustomerInvoices Where [Job Code] = 'G3093'
,
SellerInvoices AS (
    SELECT 
		aid.Code AS 'Job Code',
		...
		END AS 'Seller Invoice Amount',
		CASE 
			WHEN ais.InvoiceType = 1 AND acur.Code = 'AED' THEN ROUND(-ABS(ais.TotalAmount / 3.6725),2)
			WHEN ais.InvoiceType = 1 AND acur.Code <> 'AED' THEN ROUND(-ABS(ais.TotalAmount * aio.ExchangeRate),2)
			WHEN ais.InvoiceType = 2 AND acur.Code = 'AED' THEN ROUND(ais.TotalAmount / 3.6725, 2)
			WHEN ais.InvoiceType = 2 AND acur.Code <> 'AED' THEN ROUND(ais.TotalAmount * aio.ExchangeRate, 2)
		END AS 'Seller Invoice Total Amount',
		CASE 
			WHEN ais.InvoiceType = 1 AND acur.Code = 'AED' THEN ROUND(-ABS(ais.AmountPaidSoFar / 3.6725),2)
			WHEN ais.InvoiceType = 1 AND acur.Code <> 'AED' THEN ROUND(-ABS(ais.AmountPaidSoFar * aio.ExchangeRate),2)
			WHEN ais.InvoiceType = 2 AND acur.Code = 'AED' THEN ROUND(ais.AmountPaidSoFar / 3.6725, 2)
			WHEN ais.InvoiceType = 2 AND acur.Code <> 'AED' THEN ROUND(ais.AmountPaidSoFar * aio.ExchangeRate, 2)
		END AS 'Amount Paid',
		CASE
			WHEN aid.InquiryStatus IN (700,800) THEN 'Not paid'
			WHEN aid.InquiryStatus NOT IN (700,800)
...
Select 
	...
	SUM([Customer Invoice Total Amount]) AS CustomerTotalAmount,
	--CASE 
	...
	ROUND(SUM([Customer Invoice Total Amount]),2) AS 'Customer Invoice Total Amount',
	--CASE 
	...
	ROUND(SUM([Seller Invoice Total Amount]),2) AS 'Seller Invoice Total Amount',
 ...
 --   END AS 'Selling Currency',
    ...
    END AS 'Trade Type',
	SUM([Total Brokerage]) AS 'Total Brokerage'
...
from ExampleCTE where [Job Status] IN ('Partly Booked', 'Booked', 'Partly Delivered', 'Delivered', 'Cancelled Stem')