...
from AppInquiryMiscCosts aimc
WHERE IsDeleted = 0 and FromBuyer = 1
GROUP BY aimc.InquiryDetailId, aimc.InquirySellerDetailId
),
...
Select 
	...
	SUM(AmountUsd) AS 'Amount'
from AppInquiryMiscCosts aimc
WHERE IsDeleted = 0 and ToSeller = 1
GROUP BY aimc.InquiryDetailId, aimc.InquirySellerDetailId
),
StemDate AS (
Select 
...
Where ain.IsBooked = 1 and ain.IsNominated = 1 and ain.IsDeleted = 0 and ain.BookedOn is not null
),
GSData AS (
SELECT DISTINCT
	'GS' AS 'Data source',
	...
	COALESCE(asur.Name, agen.Name, asup.Name, abro.Name, aselll.Name, asell.Name, asel.Name) AS 'Seller name',
	...
	CONVERT(DATE, COALESCE(ais.AmountPaidDate, ais.AcknowledgementSentOn)) AS 'DatePaid',
	...
	CASE
		...
		THEN
			CASE 
				...
				WHEN aifd.Unit = 1 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.001, 4) --KG
				WHEN aifd.Unit = 2 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.00085, 4) --Litres
				WHEN aifd.Unit = 3 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.0038641765, 4) --IG
				WHEN aifd.Unit = 4 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.85, 4) --CBM
				WHEN aifd.Unit = 5 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.0032, 4) --US Gallons
				WHEN aifd.Unit = 6 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.134, 4) --Barrels
				WHEN aifd.Unit = 7 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.85, 4) --KL
			END
		WHEN agg.Name = 'FO'
		THEN
			CASE
				WHEN aifd.Unit = 0 THEN COALESCE(ain.QuantityMin, aifd.QuantityMin)	--MT
				WHEN aifd.Unit = 1 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.001, 4) --KG
				WHEN aifd.Unit = 2 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.00094, 4) --Litres
				WHEN aifd.Unit = 3 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.0042733246, 4)--IG
				WHEN aifd.Unit = 4 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.94, 4) --CBM
				WHEN aifd.Unit = 5 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.0037, 4) --US Gallons
				WHEN aifd.Unit = 6 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.157, 4) --Barrels
				WHEN aifd.Unit = 7 THEN ROUND(COALESCE(ain.QuantityMin, aifd.QuantityMin) * 0.94, 4) --KL
			END
	...
	CASE
		...
		THEN
			CASE 
				...
				WHEN ad.BdnQtyUnit = 1 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.001, 4) --KG
				WHEN ad.BdnQtyUnit = 2 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.00085, 4) --Litres
				WHEN ad.BdnQtyUnit = 3 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.0038641765, 4) --IG
				WHEN ad.BdnQtyUnit = 4 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.85, 4) --CBM
				WHEN ad.BdnQtyUnit = 5 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.0032, 4) --US Gallons
				WHEN ad.BdnQtyUnit = 6 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.134, 4) --Barrels
				WHEN ad.BdnQtyUnit = 7 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.85, 4) --KL
			END
		WHEN agg.Name = 'FO'
		THEN
			CASE
				WHEN ad.BdnQtyUnit = 0 THEN COALESCE(ais.BdnBillingQuantity, ad.BdnQty)	--MT
				WHEN ad.BdnQtyUnit = 1 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.001, 4) --KG
				WHEN ad.BdnQtyUnit = 2 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.00094, 4) --Litres
				WHEN ad.BdnQtyUnit = 3 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.0042733246, 4)--IG
				WHEN ad.BdnQtyUnit = 4 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.94, 4) --CBM
				WHEN ad.BdnQtyUnit = 5 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.0037, 4) --US Gallons
				WHEN ad.BdnQtyUnit = 6 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.157, 4) --Barrels
				WHEN ad.BdnQtyUnit = 7 THEN ROUND(COALESCE(ais.BdnBillingQuantity, ad.BdnQty) * 0.94, 4) --KL
			END
	...
	CASE
		WHEN bc.Code = 'AED' THEN ROUND(COALESCE(ais.BuyPrice / 3.6725, aim.buyprice / 3.6725),2)
		WHEN bc.Code <> 'AED' THEN ROUND(COALESCE(ais.BuyPrice * aio.ExchangeRate, aim.buypriceusd),2)
	...
	CASE	
		WHEN ais.InvoiceType = 0 and sc.Code = 'AED' THEN ROUND(aic.TotalAmount / 3.6725, 2)
		WHEN ais.InvoiceType = 0 and sc.Code <> 'AED' THEN ROUND(aic.TotalAmount * aim.ExchangeRate, 2)
		ELSE NULL
	...
	CASE	
		WHEN ais.InvoiceType = 0 and sc.Code = 'AED' THEN ROUND(aic.SubTotal / 3.6725, 2)
		WHEN ais.InvoiceType = 0 and sc.Code <> 'AED' THEN ROUND(aic.SubTotal * aim.ExchangeRate, 2)
		ELSE NULL
...
LEFT JOIN AppInquiryFuelDetails aifd ON aifd.inquirydetailid = aid.id and aifd.isdeleted = 0 and aifd.islosted = 0
JOIN AppInquirySellerDetails aisd ON aisd.InquiryFuelDetailId = aifd.id and aifd.sellerid = aisd.sellerid and aisd.isdeleted = 0
JOIN StemDate ain ON ain.InquiryDetailId = aifd.InquiryDetailId and ain.IsDeleted = 0 AND ain.InquirySellerDetailId = aisd.Id
...
LEFT JOIN AppGradeSpecs ags ON ags.id = aifd.GradeSpecId and ags.IsDeleted = 0
LEFT JOIN AppGradeSpecs agss ON agss.id = ain.GradeSpecId and agss.IsDeleted = 0
...
LEFT JOIN AppSuppliers asupp ON asupp.Id = ais.CounterpartyName and asupp.IsDeleted = 0
LEFT JOIN AppSuppliers asuppp ON asuppp.Id = ais.SupplierId and asuppp.IsDeleted = 0
LEFT JOIN AppInquiryMargins aim ON aim.InquirySellerDetailId = aisd.Id and aim.isdeleted = 0
...
LEFT JOIN InquiryMiscCostSeller ims ON ims.InquirySellerDetailId = aisd.Id and ims.InquiryDetailId = aisd.InquiryDetailId
LEFT JOIN InquiryMiscCostCustomer imc ON imc.InquirySellerDetailId = aisd.Id and imc.InquiryDetailId = aisd.InquiryDetailId
WHERE aid.InquiryStatus IN (650, 700, 800, 900, 1000, 9000)
),
HistoricalData AS (
SELECT
	...
	CASE WHEN DatePaid = 'N\A' THEN NULL ELSE DatePaid END AS DatePaid, --Converted the 'N\A' values into NULL so that the union would be satisfied.
	...
	(MiscCostItemOneAmount+MiscCostItemTwoAmount+MiscCostItemThreeAmount+MiscCostItemFourAmount+MiscCostItemFiveAmount) as 'Customer Grade Misc Cost', --new
	NULL as 'CustomerInvAddlCost', --new
	...
	(MiscCostItemOneAmount+MiscCostItemTwoAmount+MiscCostItemThreeAmount+MiscCostItemFourAmount+MiscCostItemFiveAmount) as 'Seller Grade Misc Cost', --new
	NULL as 'SellerInvAddlCost', --new
...
FROM AppHistoricalPurchaseData
),
UNIONALLCTE AS (
...
Select 
	*  
from UNIONALLCTE --WHERE [Job code] =  'G2825'
ORDER BY  
    CASE WHEN [Data Source] = 'GS' THEN 0 ELSE 1 END, 
    CASE 
    ...
    [Stem date] DESC,
    CASE 
        WHEN LEN([Job code]) > 1 THEN CAST(SUBSTRING([Job code], 2, LEN([Job code]) - 1) AS INT) 
