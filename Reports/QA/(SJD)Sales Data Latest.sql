...
SELECT 
	...
	CASE
		...
		THEN
			CASE 
				...
				WHEN ad.BdnQtyUnit = 1 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.001, 4) --KG
				WHEN ad.BdnQtyUnit = 2 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.00085, 4) --Litres
				WHEN ad.BdnQtyUnit = 3 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.0038641765, 4) --IG
				WHEN ad.BdnQtyUnit = 4 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.85, 4) --CBM
				WHEN ad.BdnQtyUnit = 5 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.0032, 4) --US Gallons
				WHEN ad.BdnQtyUnit = 6 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.134, 4) --Barrels
				WHEN ad.BdnQtyUnit = 7 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.85, 4) --KL
			END
		WHEN agg.Name = 'FO'
		THEN
			CASE
				WHEN ad.BdnQtyUnit = 0 THEN COALESCE(aic.BdnBillingQuantity, ad.BdnQty)	--MT
				WHEN ad.BdnQtyUnit = 1 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.001, 4) --KG
				WHEN ad.BdnQtyUnit = 2 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.00094, 4) --Litres
				WHEN ad.BdnQtyUnit = 3 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.0042733246, 4)--IG
				WHEN ad.BdnQtyUnit = 4 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.94, 4) --CBM
				WHEN ad.BdnQtyUnit = 5 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.0037, 4) --US Gallons
				WHEN ad.BdnQtyUnit = 6 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.157, 4) --Barrels
				WHEN ad.BdnQtyUnit = 7 THEN ROUND(COALESCE(aic.BdnBillingQuantity, ad.BdnQty) * 0.94, 4) --KL
			END
	...
	CASE 
		WHEN sel.Code = 'AED' THEN ROUND(COALESCE(aic.SellPrice / 3.6725, aim.SellPrice / 3.6725), 2)
		WHEN sel.Code <> 'AED' THEN ROUND(COALESCE(aic.SellPrice * aim.ExchangeRate, aim.SellPriceUsd), 2)
	END AS 'Selling Price(USD)',
	ROUND(COALESCE(ais.BuyPrice, aim.BuyPrice), 2) AS 'Buying price',
	buy.Code AS 'Buying Price Currency type',
	CASE 
		WHEN buy.Code = 'AED' THEN ROUND(COALESCE(ais.BuyPrice / 3.6725, aim.Buyprice / 3.6725),2)
		WHEN buy.Code <> 'AED' THEN ROUND(COALESCE(ais.BuyPrice * aio.ExchangeRate, aim.Buypriceusd),2)
	...
	CASE 
		WHEN aic.InvoiceType = 0 and buy.Code = 'AED' THEN ROUND(ais.TotalAmount / 3.6725, 2)
		WHEN aic.InvoiceType = 0 and buy.Code <> 'AED' THEN ROUND(ais.TotalAmount * aio.ExchangeRate, 2)
		ELSE NULL
	...
	CASE 
		WHEN aic.InvoiceType = 0 and buy.Code = 'AED' THEN ROUND(ais.SubTotal / 3.6725, 2)
		WHEN aic.InvoiceType = 0 and buy.Code <> 'AED' THEN ROUND(ais.SubTotal * aio.ExchangeRate, 2)
		ELSE NULL
...
JOIN StemDate ain ON ain.InquirySellerDetailId = aisd.id and ain.InquiryDetailId = aifd.InquiryDetailId and ain.IsDeleted = 0
...
LEFT JOIN AppInquiryMargins aim ON aim.InquirySellerDetailId = aisd.Id and aim.InquiryDetailId = aisd.InquiryDetailId and aim.IsDeleted = 0
...
LEFT JOIN AppCurrencies buy ON buy.Id = aim.CurrencyId and buy.IsDeleted = 0
LEFT JOIN AppCountries aco ON aco.Id = ac.JurisdictionId and aco.IsDeleted = 0
...
Select 
	* 
from UnionCTE --where [Date Received] is null and [Data source] = 'GS' and [Job status] = 'Delivered' and [Payment Status] = 'Received'
ORDER BY 
    CASE 
