...
-- Added Broker Amount And Broker Currency
Updated By : Chinthu P S
Updated On : 15-04-2025 03:11 IST

Updated By : Chinthu P S
Updated On : 16-04-2025 03:12 IST

Updated ON UAT: 25-04-2025 03:56 IST
Payables report - Broker Amount ,Broker Currency
*/
...
InquiryBroker AS (
    SELECT 
        ...
        SellerBrokerage,
		SellerCurrencyId
    FROM AppInquiryBrokerDetails 
    WHERE SellerBrokerId is not null and IsDeleted = 0
),
unionquery AS ((
    SELECT 
...
LEFT JOIN InquiryBroker aibd ON aibd.InquiryFuelDetailId = aifd.Id 
LEFT JOIN AppCurrencies ac ON ac.id =  aibd.SellerCurrencyId AND ac.IsDeleted=0
...
WHERE [Seller Name] IS NOT NULL --and [Job Number] = 'G2972'--AND [Invoice Status] <> 'Paid'
 AND 
    ...
    (@StartDate IS NULL AND TRY_CONVERT(DATE, COALESCE([Payment Due Date], [Expected Payment Date])) <= TRY_CONVERT(DATE, @EndDate))
    OR (@StartDate IS NOT NULL AND @EndDate IS NULL AND TRY_CONVERT(DATE, COALESCE([Payment Due Date], [Expected Payment Date])) >=  TRY_CONVERT(DATE, @StartDate) 
    OR (TRY_CONVERT(DATE, COALESCE([Payment Due Date], [Expected Payment Date])) BETWEEN TRY_CONVERT(DATE, @StartDate) AND TRY_CONVERT(DATE, @EndDate))))
