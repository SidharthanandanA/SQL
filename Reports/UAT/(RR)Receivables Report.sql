...
Select Id, InquirySellerDetailId, InquiryDetailId, BuyerCreditTerms, QuantityMax, BuyerPaymentTerm
...
FirstRecordPerCustomer AS (
    SELECT 
        ...
        ROW_NUMBER() OVER (PARTITION BY accc.CustomerId ORDER BY accc.CreationTime ASC) AS RowNum
    FROM 
        AppCustomerCreditCompliances accc
		LEFT JOIN AppFinancingFacilites aff ON aff.Id = accc.FinancingFacilityId and aff.IsDeleted = 0
    WHERE 
        accc.IsDeleted = 0
),
BuyerMiscCost AS (
...
unionquery AS ((
  ...
  --      END AS 'Payment Due Date',
	CONVERT(DATE, DATEADD(DAY, bpb.BuyerCreditTerms, aid.DeliveryStartDateNomination)) AS 'Payment Due Date',
  ...
  --      END AS 'Overdue days',
	...
	CASE 
		...
		WHEN bmc.BuyerMiscCost IS NOT NULL THEN ROUND(bpb.QuantityMax * aim.SellPrice + bmc.BuyerMiscCost, 2)
	END AS 'Outstanding Amount',
	CASE 
    -- WHEN bpb.BuyerPaymentTerm = 2 THEN ROUND(apc.BalanceDue, 2)
    WHEN bmc.BuyerMiscCost IS NULL THEN
        CASE
            WHEN acur.Code = 'AED' THEN ROUND(bpb.QuantityMax * aim.SellPrice / 3.6725, 2)
            ELSE ROUND(bpb.QuantityMax * aim.SellPriceUsd, 2)
        END
    WHEN bmc.BuyerMiscCost IS NOT NULL THEN
        CASE
            WHEN acur.Code = 'AED' THEN ROUND((bpb.QuantityMax * aim.SellPrice + bmc.BuyerMiscCost) / 3.6725, 2)
            ELSE ROUND(bpb.QuantityMax * aim.SellPriceUsd + bmc.BuyerMiscCost, 2)
        END
  ...
  --      END AS 'Amount Received So Far',
	...
	CASE  	
		...
		--WHEN aifd.IsCancelled = 1 THEN 'Cancelled'
...
JOIN AppInquiryMargins aim ON aim.InquirySellerDetailId = bpb.InquirySellerDetailId and aim.IsDeleted = 0
--LEFT JOIN AppInquiryFuelDetails aifd ON aifd.Id = aim.InquiryDetailId
LEFT JOIN AppProformaCustomers apc ON apc.InquiryNominationId = bpb.Id AND apc.IsDeleted = 0
...
LEFT JOIN FirstRecordPerCustomer frpc ON frpc.CustomerId = ac.Id and frpc.RowNum = 1
...
SELECT DISTINCT
	...
	CASE 
		WHEN aic.AmountReceivedSoFar IS NULL THEN 
		    CASE
	...
	END AS 'Amount Received So Far',
	CASE 
    WHEN acur.Code = 'AED' THEN 
        CASE 
            WHEN aic.AmountReceivedSoFar IS NULL THEN 
                CASE
                    WHEN apc.AmountReceived IS NULL THEN ROUND((aic.TotalAmount - (aic.BalanceDue)) / 3.6725, 2)
                    WHEN apc.AmountReceived IS NOT NULL THEN ROUND((aic.TotalAmount - (aic.BalanceDue + apc.AmountReceived)) / 3.6725, 2)
                END
    ...
    WHEN acur.Code <> 'AED' THEN 
        CASE 
            WHEN aic.AmountReceivedSoFar IS NULL THEN 
                CASE
                    WHEN apc.AmountReceived IS NULL THEN ROUND((aic.TotalAmount - (aic.BalanceDue)) * aim.ExchangeRate, 2)
                    WHEN apc.AmountReceived IS NOT NULL THEN ROUND((aic.TotalAmount - (aic.BalanceDue + apc.AmountReceived)) * aim.ExchangeRate, 2)
                END
...
LEFT JOIN AppInquiryCancelStems aics ON aifd.Id = aics.InquiryFuelDetailId and aics.CancelTypes = 0
JOIN AppCustomers acus ON acus.Id = aid.CustomerNominationId
LEFT JOIN AppCustomerGroups acg ON acg.id = acus.CustomerGroupId
LEFT JOIN FirstRecordPerCustomer frpc ON frpc.CustomerId = acus.Id and frpc.RowNum = 1
LEFT JOIN AppCustomerCreditCompliances accc ON acus.Id = accc.CustomerId and accc.IsDeleted = 0
LEFT JOIN AppInvoiceCustomers aic ON aic.InquiryFuelDetailId = aifd.Id and aic.IsDeleted = 0
LEFT JOIN AppProformaCustomers apc ON apc.inquiryfueldetailid = aic.InquiryFuelDetailId and aic.IsDeleted = 0 --test
JOIN AppVessel av ON av.Id = aid.VesselNominationId
JOIN AppPorts ap ON ap.Id = aid.PortNominationId
...
JOIN AppCurrencies acur ON aim.CurrencyId = acur.Id and aim.Id = aic.InquiryMarginId
where aid.InquiryStatus = 700 or aid.InquiryStatus = 800 or aid.InquiryStatus = 900 or aid.InquiryStatus = 1000 or aid.InquiryStatus = 9000 and aics.CancelTypes = 0 and aics.IsDeleted = 0
)),
--Select 
--	* 
--from unionquery
...
CombinedDateRanked AS (
    SELECT
        *,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY 
                COALESCE([Invoice Number], [Job Number]) -- Use Job Number when Invoice Number is NULL
            ORDER BY [Payment Due Date] ASC
...
AggregatedData AS (
    SELECT 
        ...
        DATEDIFF(day, MinPaymentDueDate, GETDATE()) AS [Overdue days], -- Calculate Overdue days based on MinPaymentDueDate
        SUM([Outstanding Amount]) AS [Outstanding Amount], -- Sum Outstanding Amount
		SUM([Outstanding Amount(USD)]) AS [Outstanding Amount(USD)], -- Sum Outstanding Amount(USD)
        SUM([Invoice Amount]) AS [Invoice Amount], -- Sum Invoice Amount
		SUM([Invoice Amount(USD)]) AS [Invoice Amount(USD)], -- Sum Invoice Amount(USD)
        SUM([Amount Received So Far]) AS [Amount Received So Far], -- Sum Amount Received So Far
		SUM([Amount Received So Far(USD)]) AS [Amount Received So Far(USD)], -- Sum Amount Received So Far
        -- For Delivery Date, pick the value corresponding to the MinPaymentDueDate
        MAX(CASE WHEN [Payment Due Date] = MinPaymentDueDate THEN [Delivery Date]
				 WHEN [Payment Due Date] <> MinPaymentDueDate THEN [Delivery Date] END) AS [Delivery Date], 
    ...
    GROUP BY 
        ...
        [Job Number],
        [Invoice Number],
        MinPaymentDueDate, -- Include MinPaymentDueDate in the GROUP BY clause
		MinInvoiceDate,
		[Expected Payment Received Date],
        [Currency],
        [Invoice Status],
...
WHERE 
    ...
    (@StartDate IS NULL AND TRY_CONVERT(DATE, COALESCE([Payment Due Date], [Expected Payment Received Date])) <= TRY_CONVERT(DATE, @EndDate))
    OR (@StartDate IS NOT NULL AND @EndDate IS NULL AND TRY_CONVERT(DATE, COALESCE([Payment Due Date], [Expected Payment Received Date])) >=  TRY_CONVERT(DATE, @StartDate) 
    OR (TRY_CONVERT(DATE, COALESCE([Payment Due Date], [Expected Payment Received Date])) BETWEEN TRY_CONVERT(DATE, @StartDate) AND TRY_CONVERT(DATE, @EndDate))))
ORDER BY CAST(SUBSTRING([Job Number], 2, LEN([Job Number]) - 1) AS INT) DESC