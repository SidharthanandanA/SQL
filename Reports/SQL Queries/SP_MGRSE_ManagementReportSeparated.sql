-- ===================================================
-- Procedure Name: SP_MGRSE_ManagementReportSeparated
...
CustomerInvoices AS (
    SELECT
        ...
        CASE
            ...
            AND acur.Code = 'AED' THEN ROUND(- ABS(aic.SubTotal / 3.6725), 2)
            WHEN aic.InvoiceType = 1
            AND acur.Code <> 'AED' THEN ROUND(- ABS(aic.SubTotal * aim.ExchangeRate), 2)
            WHEN aic.InvoiceType = 2
            AND acur.Code = 'AED' THEN ROUND(aic.SubTotal / 3.6725, 2)
            WHEN aic.InvoiceType = 2
            AND acur.Code <> 'AED' THEN ROUND(aic.SubTotal * aim.ExchangeRate, 2)
        END AS 'Customer Invoice Amount USD',
        CASE
...
SellerInvoices AS (
    SELECT
        ...
        CASE
            ...
            AND acur.Code = 'AED' THEN ROUND(- ABS(ais.SubTotal / 3.6725), 2)
            WHEN ais.InvoiceType = 1
            AND acur.Code <> 'AED' THEN ROUND(- ABS(ais.SubTotal * aio.ExchangeRate), 2)
            WHEN ais.InvoiceType = 2
            AND acur.Code = 'AED' THEN ROUND(ais.SubTotal / 3.6725, 2)
            WHEN ais.InvoiceType = 2
            AND acur.Code <> 'AED' THEN ROUND(ais.SubTotal * aio.ExchangeRate, 2)
        END AS 'Seller Invoice Amount USD',
        CASE
