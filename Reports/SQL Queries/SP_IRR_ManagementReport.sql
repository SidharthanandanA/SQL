    ...
    SELECT
        ...
        CASE
            WHEN agg.Name = 'GO' THEN CASE
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 1 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 2 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 3 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 4 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 5 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 6 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                ...
                WHEN COALESCE(ad.BdnQtyUnit, aifd.Unit) = 7 THEN ROUND(
                    COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
        ...
        CASE
            ...
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND (aifd.IsDelivered = 1 OR aifd.IsCancelled = 1))
            AND COALESCE(cusCur1.Code, cusCur2.Code) = 'AED' THEN ROUND(aic.TotalAmount / 3.6725, 2)
            ELSE ROUND(aic.TotalAmount * aic.ExchangeRate, 2)
        ...
        CASE
            ...
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND (aifd.IsDelivered = 1 OR aifd.IsCancelled = 1))
            AND COALESCE(selCur1.Code, selCur2.Code) = 'AED' THEN ROUND(ais.TotalAmount / 3.6725, 2)
            ELSE ROUND(ais.TotalAmount * ais.ExchangeRate, 2)
        END AS 'Seller Invoice Total Amount',
        CASE
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND aifd.IsDelivered = 0) THEN NULL
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND (aifd.IsDelivered = 1 OR aifd.IsCancelled = 1))
            AND COALESCE(cusCur1.Code, cusCur1.Code) = 'AED' THEN ROUND(aic.AmountRecievedSoFar / 3.6725, 2)
            ELSE ROUND(aic.AmountRecievedSoFar * aic.ExchangeRate, 2)
        END AS 'Amount Received',
        CASE
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND aifd.IsDelivered = 0) THEN NULL
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND (aifd.IsDelivered = 1 OR aifd.IsCancelled = 1))
            AND COALESCE(selCur1.Code, selCur2.Code) = 'AED' THEN ROUND(ais.AmountPaidSoFar / 3.6725, 2)
            ELSE ROUND(ais.AmountPaidSoFar * ais.ExchangeRate, 2)
        END AS 'Amount Paid',
        CASE
...
) --Select * from MainAndBookedInvoices WHERE [Job Code] = 'G4294'
,
CustomerInvoices AS (
    SELECT
        aid.Code AS 'Job Code',
        ...
        CASE
            ...
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(- ABS(aic.SubTotal / 3.6725), 2)
            WHEN aic.InvoiceType = 1
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(- ABS(aic.SubTotal * aim.ExchangeRate), 2)
            WHEN aic.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(aic.SubTotal / 3.6725, 2)
            WHEN aic.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(aic.SubTotal * aim.ExchangeRate, 2)
        END AS 'Customer Invoice Amount',
        CASE
            WHEN aic.InvoiceType = 1
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(- ABS(aic.TotalAmount / 3.6725), 2)
            WHEN aic.InvoiceType = 1
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(- ABS(aic.TotalAmount * aic.ExchangeRate), 2)
            WHEN aic.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(aic.TotalAmount / 3.6725, 2)
            WHEN aic.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(aic.TotalAmount * aic.ExchangeRate, 2)
        END AS 'Customer Invoice Total Amount',
        CASE
            WHEN aic.InvoiceType = 1
            AND acur.Code = 'AED' THEN ROUND(- ABS(aic.AmountReceivedSoFar / 3.6725), 2)
            WHEN aic.InvoiceType = 1
            ...
            WHEN aic.InvoiceType = 2
            AND acur.Code = 'AED' THEN ROUND(aic.AmountReceivedSoFar / 3.6725, 2)
            WHEN aic.InvoiceType = 2
            AND acur.Code <> 'AED' THEN ROUND(aic.AmountReceivedSoFar * aic.ExchangeRate, 2)
        END AS 'Amount Received',
        CASE
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND aifd.IsDelivered = 0) THEN 'Not received'
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND (aifd.IsDelivered = 1 OR aifd.IsCancelled = 1)) THEN CASE
        ...
        CASE
            ...
            AND aic.InvoiceType = 1 THEN ROUND(
                (
                    - ABS(aic.SubTotal) / COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                    )
            ...
            WHEN aic.SubTotal IS NOT NULL
            AND COALESCE(ad.BdnQtyUnit, aifd.Unit) IS NOT NULL
            AND aic.InvoiceType = 2 THEN ROUND(
                (
                    aic.SubTotal / COALESCE(
                        aic.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                    )
...
SellerInvoices AS (
    SELECT
        ...
        CASE
            ...
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(- ABS(ais.SubTotal / 3.6725), 2)
            WHEN ais.InvoiceType = 1
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(- ABS(ais.SubTotal * ais.ExchangeRate), 2)
            WHEN ais.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(ais.SubTotal / 3.6725, 2)
            WHEN ais.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(ais.SubTotal * ais.ExchangeRate, 2)
        END AS 'Seller Invoice Amount',
        CASE
            WHEN ais.InvoiceType = 1
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(- ABS(ais.TotalAmount / 3.6725), 2)
            WHEN ais.InvoiceType = 1
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(- ABS(ais.TotalAmount * ais.ExchangeRate), 2)
            WHEN ais.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) = 'AED' THEN ROUND(ais.SubTotal / 3.6725, 2)
            WHEN ais.InvoiceType = 2
            AND COALESCE(acurr.Code, acur.Code) <> 'AED' THEN ROUND(ais.TotalAmount * ais.ExchangeRate, 2)
        END AS 'Seller Invoice Total Amount',
        CASE
            WHEN ais.InvoiceType = 1
            AND acur.Code = 'AED' THEN ROUND(- ABS(ais.AmountPaidSoFar / 3.6725), 2)
            WHEN ais.InvoiceType = 1
            AND acur.Code <> 'AED' THEN ROUND(- ABS(ais.AmountPaidSoFar * ais.ExchangeRate), 2)
            WHEN ais.InvoiceType = 2
            AND acur.Code = 'AED' THEN ROUND(ais.AmountPaidSoFar / 3.6725, 2)
            WHEN ais.InvoiceType = 2
            AND acur.Code <> 'AED' THEN ROUND(ais.AmountPaidSoFar * ais.ExchangeRate, 2)
        END AS 'Amount Paid',
        CASE
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND aifd.IsDelivered = 0) THEN 'Not paid'
            WHEN (aifd.isBooked = 1 AND aifd.IsNominated = 1 AND (aifd.IsDelivered = 1 OR aifd.IsCancelled = 1)) THEN CASE
        ...
        CASE
            ...
            AND COALESCE(ad.BdnQtyUnit, aifd.Unit) IS NOT NULL
            AND ais.InvoiceType = 1 THEN ROUND(
                (
                    - ABS(ais.SubTotal) / COALESCE(
            ...
            WHEN ais.SubTotal IS NOT NULL
            AND COALESCE(ad.BdnQtyUnit, aifd.Unit) IS NOT NULL
            AND ais.InvoiceType = 2 THEN ROUND(
                (
                    ais.SubTotal / COALESCE(
                        ais.BdnBillingQuantity,
                        ad.BDNQty,
                        ain.QuantityMax
                    )
...
) --Select * from SellerInvoices WHERE [Job Code] = 'G4294'
,
CnAndDnInvoices AS (
    SELECT
